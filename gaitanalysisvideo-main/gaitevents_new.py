...
def simple_chart(raw_data, fps):
    ...
    time_axis = np.linspace(0, len(data_smoothed)-1/fps, num=len(data_smoothed))
...
def front_analysis(gaitdata):
    ...
    csvkinetime = {Side.LEFT: open('kinematics_3.csv','a', newline=''),
                   Side.RIGHT: open('kinematics_4.csv','a', newline='')}
...
def cbta(gaitdata,file_name,all_timings):
    #JSLOW(20230807) - manually selected HS TO frames
    if hasattr(gaitdata,"manualHS") and hasattr(gaitdata,"manualTO"):
    ...
    print_analysis_HSTO(file_name, all_timings, HS, TO, True, gaitdata)
...
def print_analysis_HSTO(file_name,all_timings, HS, TO, use_gaitdata: bool, gaitdata = None, fps=50, kinedata={}):
    stride_timings = {Side.LEFT: all_timings[file_name][1][0], Side.RIGHT: all_timings[file_name][1][1]}
    ...
    if use_gaitdata:
        ...
        filt_shoulder = {Side.LEFT: [], Side.RIGHT: []}
    ...
    csvfile = open('gaitanalysis_cbta.csv', 'a', newline='')
    ...
    for side in Side:
        ...
        csvwriter.writerow([f'HS,{side.name}',HS[side]])
        csvwriter.writerow([f'TO,{side.name}',TO[side]])
        csvwriter.writerow([f"tohs_pairs,{side.name}", tohs_pairs[side]])
        csvwriter.writerow([f"hsto_pairs,{side.name}", hsto_pairs_alt[side]])
        ...
        for i in range(len(stride_pairs[side])):
            ...
            #tempcsvwriter.writerow(['Foot off (%)', str(foot_off)])
            #tempcsvwriter.writerow(['Limp index', str(limp_index)])
            #tempcsvwriter.writerow(['Opposite foot contact (%)', str(opp_foot_contact)])
            #tempcsvwriter.writerow(['Opposite foot off (%)', str(opp_foot_off)])
            ...
            for k in range(len(step_pairs[side])):
                if is_pair_inside_pair(step_pairs[side][k], stride_pairs[side][i]):
                    #tempif use_gaitdata:
                        #tempcsvwriter.writerow(['Step length (m)', str(step_lengths[side][k])])
                    #tempcsvwriter.writerow(['Step time (s)', str(step_durations[side][k])])
                    stepDuration = step_durations[side][k]
            #tempif use_gaitdata:
                #tempcsvwriter.writerow(['Stride length (m)', str(stride_lengths[side][i])])
            ...
            #tempif use_gaitdata:
                ...
                #tempcsvwriter.writerow(['Walking speed (m/s)', str(stride_lengths[side][i]/stride_durations[side][i])])
            #JSLOW(20230719) - change cadence to per stride
            #  csvwriter.writerow(['Cadence (step/min)', str(cadence * 60)])
            #tempif stepDuration > 0:
                #tempcsvwriter.writerow(['Cadence (step/min)', str(60/stepDuration)])
            #tempelse:
                #tempcsvwriter.writerow(['Cadence (step/min)', '0'])
    ...
    #tempcsvwriter.writerow(['Average Cadence', str(cadence * 60)])
    ...
    if use_gaitdata:
        ...
        knee_flexion1 = {Side.LEFT: [], Side.RIGHT: []}
        hip_flexion1 = {Side.LEFT: [], Side.RIGHT: []}
        ankle_flexion1 = {Side.LEFT: [], Side.RIGHT: []}
        line_style = {Side.LEFT: "r", Side.RIGHT: "g"}
        shoulder_distance = {Side.LEFT: [], Side.RIGHT: []}
# JSLOW(20230216) - add csv output for kinematic data per frame per side
        csvkinetime = {Side.LEFT: open('kinematics_1.csv','a', newline=''),
                       Side.RIGHT: open('kinematics_2.csv','a', newline='')}
        cktwriter = {Side.LEFT: csv.writer(csvkinetime[Side.LEFT], lineterminator='\n'), 
                    Side.RIGHT: csv.writer(csvkinetime[Side.RIGHT], lineterminator='\n')}
        ...
        for side in Side:
            ...
            for i in range(len(time_axis)):
                ...
                shoulder_distance[side].append(filt_shoulder[Side.LEFT][i][0]-filt_shoulder[Side.RIGHT][i][0])
                if gaitdata.framework != 'vicon':
            ...
            cktwriter[side].writerow([file_name])
            cktwriter[side].writerow(["time","ankle_flexion", "knee_flexion", "hip_flexion","confidence"])
            end_time = 0
            for i,time in enumerate(time_axis):
                if time <= float(stride_timings[side][0])/fps:
                    start_time = i
                else:
                    break
            for j,time in enumerate(time_axis):
                if time >= float(stride_timings[side][1])/fps and end_time==0:
                    end_time = j
            if end_time<start_time:
                cktwriter[side].writerow(['ERROR, check HS timings'])       
            
            posescore = gaitdata.poseScore[start_time:end_time+1]
            time_axis1 = time_axis[start_time:end_time+1]
            ankle_flexion1[side] = ankle_flexion[side][start_time:end_time+1]
            knee_flexion1[side] = knee_flexion[side][start_time:end_time+1]
            hip_flexion1[side] = hip_flexion[side][start_time:end_time+1]
            flexions = np.array([time_axis1, ankle_flexion1[side], knee_flexion1[side], hip_flexion1[side],posescore]).T
